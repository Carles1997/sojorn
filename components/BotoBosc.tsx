...
// Botó sobre bosc: majúscules amb tracking obert, vora de paper al 30% i una línia
...
export function BotoBosc({ href, children, exterior = false, className = "" }: BotoBoscProps) {
  return (
    <a
      ...
      className={`group inline-flex items-center border border-paper/30 bg-bosc px-6 py-4 text-center text-ui tracking-[0.14em] text-paper uppercase transition-colors duration-200 ease-sojorn hover:border-or sm:px-8 sm:whitespace-nowrap ${className}`}
    >
      <span className="relative">
        {children}
        <span
          aria-hidden="true"
          className="absolute inset-x-0 -bottom-1.5 h-px origin-left scale-x-0 bg-or transition-transform duration-500 ease-sojorn group-hover:scale-x-100 group-focus-visible:scale-x-100 motion-reduce:scale-x-100 motion-reduce:opacity-0 motion-reduce:transition-opacity motion-reduce:duration-200 motion-reduce:group-hover:opacity-100 motion-reduce:group-focus-visible:opacity-100"
        />
      </span>
